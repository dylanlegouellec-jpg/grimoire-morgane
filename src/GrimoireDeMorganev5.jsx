...
function AddRecipeChoiceModal({ onClose, onManual, onTextImport, onJsonImport }) {
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal grimoire-page" onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><X size={20} /></button>
        <h2 className="dropcap-title">Nouvelle recette</h2>
        <Flourish />
        <p className="hint" style={{ fontStyle: "normal" }}>Comment veux-tu l'ajouter à ton Grimoire ?</p>
        <div className="add-choice-list">
          <Seal tone="gold" onClick={onManual}><Wand2 size={16} /> Écrire à la main</Seal>
          <Seal tone="gold" onClick={onTextImport}><FileText size={16} /> Importer ma fiche texte</Seal>
          <Seal tone="gold" onClick={onJsonImport}><Upload size={16} /> Importer un JSON</Seal>
        </div>
      </div>
    </div>
  );
...
function JsonImportModal({ onClose, onImport }) {
  ...
  const submit = () => {
    try {
      ...
      if (!obj || typeof obj !== "object" || !obj.title) {
        setError("Le JSON doit contenir au moins un champ \"title\".");
        return;
      }
      onImport(obj);
      onClose();
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal grimoire-page form-clean" onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><X size={20} /></button>
        <h2 className="dropcap-title">Importer un JSON</h2>
        <Flourish />
        <p className="hint" style={{ fontStyle: "normal" }}>Colle un objet JSON de recette (title, category, ingredients, steps…).</p>
        <textarea
          rows={12}
          className="template-textarea"
          value={text}
          onChange={(e) => { setText(e.target.value); setError(""); }}
          placeholder={'{\n  "title": "Kouign-amann",\n  "category": "Sucré",\n  "servings": 8,\n  "time": 70,\n  "ingredients": [{ "qty": 250, "unit": "g", "name": "Farine" }],\n  "steps": ["Pétrir la pâte…"]\n}'}
        />
        {error && <p className="import-error">{error}</p>}
        <Seal tone="gold" onClick={submit}><Upload size={16} /> Importer</Seal>
      </div>
    </div>
  );
}
...
function RecipeCard({ recipe, onOpen, onToggleFavorite, onRequestDelete }) {
      ...
      >
        ...
        <div className="portions-adjuster">
          ...
          <div className="portions-stepper">
            <button type="button" onClick={() => setServings((s) => Math.max(1, Number(s) - 1))}><Minus size={14} /></button>
            <span>{servings}</span>
            <button type="button" onClick={() => setServings((s) => Number(s) + 1)}><Plus size={14} /></button>
          </div>
        </div>
...
function RecipeForm({ onClose, onSave, onDelete, initialRecipe }) {
  ...
  const [ingredientRows, setIngredientRows] = useState(() =>
    initialRecipe && initialRecipe.ingredients.length
      ? initialRecipe.ingredients.map((i) => ({ id: newRowId(), qty: i.qty, unit: i.unit, name: i.name }))
  ...
  const [stepRows, setStepRows] = useState(() =>
    initialRecipe && initialRecipe.steps.length
      ? initialRecipe.steps.map((s) => ({ id: newStepId(), text: s }))
      : [{ id: newStepId(), text: "" }]
  );
  ...
  const updateStepRow = (id, value) => {
    setStepRows((prev) => prev.map((r) => (r.id === id ? { ...r, text: value } : r)));
  };

  const hasTitle = title.trim().length > 0;
  const hasIngredients = ingredientRows.some((r) => r.name.trim().length > 0);
  const hasSteps = stepRows.some((r) => r.text.trim().length > 0);
  ...
  const submit = (e) => {
    ...
    const steps = stepRows.map((r) => r.text.trim()).filter(Boolean);
      ...
      >
        ...
        <div className="step-rows">
          {stepRows.map((row, idx) => (
            <div className="step-row" key={row.id}>
              <span className="step-row-num">{idx + 1}</span>
              <input
                type="text"
                className="step-text"
                value={row.text}
                onChange={(e) => updateStepRow(row.id, e.target.value)}
        ...
        <button type="button" className="link-btn add-step-btn" onClick={addStepRow}>
          + Ajouter une étape
        </button>
    ...
    <div className="view">
      {filtered.length === 0 ? (
        ...
        <div className="recipes-grid">
          {filtered.map((r) => (
            <RecipeCard key={r.id} recipe={r} onOpen={onOpen} onToggleFavorite={onToggleFavorite} onRequestDelete={onRequestDelete} />
          ))}
        </div>
      )}
  ...
  const importRecipe = (parsed, successMessage) => {
    saveRecipe({
      ...
      ingredients: Array.isArray(parsed.ingredients) && parsed.ingredients.length
        ? parsed.ingredients.map((i) => ({ qty: Number(i.qty) || 0, unit: i.unit || "", name: String(i.name || "").trim() }))
        : [{ qty: 1, unit: "", name: "Ingrédient à préciser" }],
      steps: Array.isArray(parsed.steps) && parsed.steps.length ? parsed.steps.map((s) => String(s).trim()).filter(Boolean) : ["Étape à préciser"],
    });
    ...
    <div className="grimoire-app">
      ...
      <main className="app-content" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        {tab === "recettes" && (
          <RecipesView
            ...
            onAddRequest={() => setShowAddChoice(true)}
      ...
      {showAddChoice && (
        <AddRecipeChoiceModal
          ...
          onManual={() => { setShowAddChoice(false); setFormTarget("new"); }}
          onTextImport={() => { setShowAddChoice(false); setShowTemplateImport(true); }}
          onJsonImport={() => { setShowAddChoice(false); setShowJsonImport(true); }}
        />
      )}
      {showTemplateImport && (
      ...
      {showJsonImport && (
        <JsonImportModal
          ...
          onImport={(parsed) => importRecipe(parsed, "Recette importée !")}
        />
      )}
      {showSecretSettings && (
        <SecretSettingsModal
