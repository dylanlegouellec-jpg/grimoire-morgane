...
/*
  ...
  create table app_state (
    ...
    shopping_selected jsonb default '[]'
  );
...
function SecretSettingsModal({ onClose, onExport, onImportFile, onImportTextRecipe }) {
...
function RecipeCard({ recipe, onOpen, onToggleFavorite, onRequestDelete, enterDelay = 0 }) {
...
function CookMode({ recipe, onClose }) {
  ...
  // Le mode cuisine est sombre : on applique la même couleur de fond au
  // conteneur racine (html/body) pour éviter que le crème de base de
  // l'app n'apparaisse pendant le rebond de scroll sur iOS.
  useEffect(() => {
...
function RecipeForm({ onClose, onSave, onDelete, initialRecipe }) {
  ...
  const flashMoved = (id) => {
    setJustMovedId(id);
    setTimeout(() => setJustMovedId((cur) => (cur === id ? null : cur)), 420);
  };
  const moveIngredientRow = (id, dir) => {
    triggerHaptic(12);
  ...
  // Maintenir "+ Ajouter…" pendant 2s ajoute un titre de section au lieu d'une ligne normale.
  const useLongPressAdd = (onShortPress, onLongPress) => {
    const timer = useRef(null);
    const fired = useRef(false);
    const start = () => {
      fired.current = false;
      timer.current = setTimeout(() => { fired.current = true; onLongPress(); }, 2000);
    };
  ...
  const ingredientAddPress = useLongPressAdd(addIngredientRow, addIngredientSectionRow);
  const stepAddPress = useLongPressAdd(addStepRow, addStepSectionRow);
      ...
      >
        ...
        <div className="ingredient-rows">
          {ingredientRows.map((row, idx) =>
            row.isSection ? (
              <div className={`ingredient-section-row ${justMovedId === row.id ? "row-moved" : ""}`} key={row.id}>
                <div className="row-handles">
                  <button type="button" className="row-handle-btn" disabled={idx === 0} onClick={() => moveIngredientRow(row.id, -1)} aria-label="Monter">▲</button>
                  <button type="button" className="row-handle-btn" disabled={idx === ingredientRows.length - 1} onClick={() => moveIngredientRow(row.id, 1)} aria-label="Descendre">▼</button>
                </div>
                <input
                  type="text"
              ...
              <div className={`ingredient-row ${justMovedId === row.id ? "row-moved" : ""}`} key={row.id}>
                <div className="row-handles">
                  <button type="button" className="row-handle-btn" disabled={idx === 0} onClick={() => moveIngredientRow(row.id, -1)} aria-label="Monter">▲</button>
                  <button type="button" className="row-handle-btn" disabled={idx === ingredientRows.length - 1} onClick={() => moveIngredientRow(row.id, 1)} aria-label="Descendre">▼</button>
                </div>
        ...
        <button type="button" className="link-btn add-ingredient-btn" {...ingredientAddPress}>
          + Ajouter un ingrédient <span className="long-press-hint">(maintenir 2s : titre de section)</span>
        </button>
        ...
        <div className="step-rows">
          {stepRows.map((row, idx) =>
            row.isSection ? (
              <div className={`step-section-row ${justMovedId === row.id ? "row-moved" : ""}`} key={row.id}>
                <div className="row-handles">
                  <button type="button" className="row-handle-btn" disabled={idx === 0} onClick={() => moveStepRow(row.id, -1)} aria-label="Monter">▲</button>
                  <button type="button" className="row-handle-btn" disabled={idx === stepRows.length - 1} onClick={() => moveStepRow(row.id, 1)} aria-label="Descendre">▼</button>
                </div>
                <input
                  type="text"
              ...
              <div className={`step-row ${justMovedId === row.id ? "row-moved" : ""}`} key={row.id}>
                <div className="row-handles">
                  <button type="button" className="row-handle-btn" disabled={idx === 0} onClick={() => moveStepRow(row.id, -1)} aria-label="Monter">▲</button>
                  <button type="button" className="row-handle-btn" disabled={idx === stepRows.length - 1} onClick={() => moveStepRow(row.id, 1)} aria-label="Descendre">▼</button>
                </div>
        ...
        <button type="button" className="link-btn add-step-btn" {...stepAddPress}>
          + Ajouter une étape <span className="long-press-hint">(maintenir 2s : titre de section)</span>
        </button>
...
function RecipesView({ recipes, filter, search, favoritesOnly, onToggleFavorite, onAddRequest, onOpen, onRequestDelete }) {
...
function FridgeView({ recipes, pantry, setPantry, basics, onMoveBasicToVariable, onRemoveBasic, onResetPantry, onOpen }) {
  ...
  return (
    <div className="view">
      ...
      <div className="basics-grid">
        ...
        {basics.length === 0 && <p className="hint" style={{ margin: 0 }}>Aucun basique pour l'instant.</p>}
      </div>
...
function SwipeFlourish({ onSwipeRight, onSwipeLeft }) {
  ...
  const endGesture = (e) => {
    if (startRef.current == null) { draggingRef.current = false; setDragX(0); return; }
  ...
  return (
    <div
      ...
      onPointerLeave={() => { if (draggingRef.current) cancelGesture(); }}
    >
      ❦
    </div>
  );
...
function ShoppingView({ recipes, selected, setSelected, list, setList, showToast }) {
  ...
  const bought = [...list.filter((i) => i.checked)].sort((a, b) => a.name.localeCompare(b.name, "fr"));
  const grouped = unchecked.reduce((acc, item) => {
    acc[item.aisle] = acc[item.aisle] || [];
    acc[item.aisle].push(item);
    return acc;
  }, {});
  Object.values(grouped).forEach((items) => items.sort((a, b) => a.name.localeCompare(b.name, "fr")));
...
export default function GrimoireDeMorgane() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setShoppingSelected((state && state.shopping_selected) || []);
        setShoppingList((state && state.shopping_list) || []);
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => { if (ready && SUPABASE_READY) saveAppState({ shopping_selected: shoppingSelected }); }, [shoppingSelected, ready]);
  useEffect(() => { if (ready && SUPABASE_READY) saveAppState({ shopping_list: shoppingList }); }, [shoppingList, ready]);
  ...
  const removeBasic = (name) => {
    setBasics((prev) => prev.filter((b) => b !== name));
    setShoppingList((prev) => [{ id: nextId(), name, qty: 1, unit: "", checked: false, aisle: guessAisle(name) }, ...prev]);
  ...
  const importRecipe = (parsed, successMessage) => {
    saveRecipe({
      id: nextId(),
      title: parsed.title,
      category: parsed.category === "Sucré" ? "Sucré" : "Salé",
      ...
      <nav className="bottom-nav">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button
            ...
            onClick={() => { triggerHaptic(10); setTab(key); }}
          >
            <Icon size={20} />
            <span>{label}</span>
          </button>
...
.row-handles { display: flex; flex-direction: column; gap: 2px; flex-shrink: 0; }
.row-handle-btn {
  width: 20px; height: 16px; border: none; border-radius: 3px;
  background: rgba(179,135,42,0.15); color: var(--gold); font-size: 8px; line-height: 1;
  display: flex; align-items: center; justify-content: center; cursor: pointer;
}
.row-handle-btn:disabled { opacity: 0.25; cursor: not-allowed; }
.row-moved { animation: rowPulse 0.42s ease; border-radius: 8px; }
@keyframes rowPulse {
  0% { background: rgba(179,135,42,0.28); }
