...
function AppleSwipeControl({ onSwipeRight, onSwipeLeft }) {
  ...
  return (
    <div className={`apple-swipe ${hintClass}`}>
      ...
      <span className="apple-hint">← réinitialiser · copier →</span>
    </div>
  );
}
...
.apple-icon {
  font-size: 2.6rem; line-height: 1; cursor: grab; touch-action: pan-y;
  transition: transform 0.05s linear;
  filter: drop-shadow(0 4px 6px rgba(0,0,0,0.25));
}
.apple-hint {
  font-family: 'Cinzel', serif; font-size: 0.58rem; letter-spacing: 1px;
  color: var(--ink-soft); text-transform: uppercase; opacity: 0.7;
}
